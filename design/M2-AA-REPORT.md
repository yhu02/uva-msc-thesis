# M2 report — A/A calibration block + power analysis: **PASS (with analysis decisions pending)**
...
> **The remaining decisions are in §M2 analysis decisions (D1–D7).**

## Block design (protocol, instantiated)

The protocol calls for ≥3 identical-placement session pairs through the full
...
randomize out). Two instantiation choices go beyond the protocol's A/A text
and are recorded here as one-sentence wording clarifications:
...
## Verdict — A/A functions

**1. Qualitative pipeline sanity check: PASS, with a structural caveat.**
The canonical analysis (per-pair Wilcoxon at the analysis unit — one
...
the analysis-unit tests cannot reach α=0.05 at this block size and their
...
belongs in the D1 scope note. Note also these are
the canonical
*pipeline* metrics, not the metric forms the primary tests use — in particular
`udp_conntrack_drop_pct` is a ratio form of the drop, and H2(a) is defined
on **absolute** drops because the packed
arm's near-zero pool makes ratios ill-defined; the D4 consolidation must
align the canonical script's metric forms with the primary tests. Tainted-iteration exclusion in the canonical script applies to the
...
supplementary iteration-level *sensitivity* pairing (not the analysis
...
instrument H5 replaces is as unreliable on this data as the design
...
user-route error rate (the availability-face bands above). The design ties
...
the H4 δs as well — recorded here as a wording clarification.
...
- **H2(a) (paired Wilcoxon, spread−packed UDP drop):** the effect to
  ...
  **Wilcoxon attainability (one-sided, the hypothesized direction): n≥5 for
...
- **H3 (interaction MDE at n=8/cell, α=0.0125):** ~0.8 pods trough
  depth / ~0.26 error rate — both inside the design's "margin ≥ A/A noise
...
on the C1 session count; revisit if the sub-score definitions are set in a
form that admits a pilot estimate.

## Findings log (rule: significant A/A finding → investigate → fix → rerun)
...
- **F1 — iteration-level offset, pair 1, during-chaos EW p95 (p=0.007).**
  ...
  11/15 cells). At the analysis unit (condition-level medians) the same
  ...
  which the session-as-unit analyses absorb by design; it is in
  the variance components feeding every margin. The design does not pin
  *which tests count* as "a statistically significant A/A finding" — the
  no-fix disposition below therefore needs both a sign-off (D1) and a
  scope note for the rule (analysis-unit tests vs any pipeline output).
  **Disposition (proposed): not a pipeline defect — no fix, no rerun;
  recorded here.**
- **F2 — the "pre-window UDP slope ≈ 0" taint rule is unworkable
  ...
  most interior/high-f iterations. Needs redefinition (D3).
- **F3 — infrastructure wedge, session 6, condition f-100 (~14:10–14:30).**
  ...
  analysis-unit test was n.s. with or without it3. Residual judgment call
...
- **F4 — analysis defects found & fixed during the block (transparency).**
  ...
  *this report* transcribed several variance-table rows from the
  ...
  regenerated wholesale from the taint-excluded artifact. The literal
  taint convention is "no result is ever quoted from a rejected or tainted
  **session**"; excluding tainted *iterations* (the earlier pipeline convention,
  and what `m2_aa_analysis.py` implements) is an interpretation that must be
  pinned — under the literal session-level reading, session 6
  would be wholly excluded and pair 3 invalid. Folded into D2.

## M2 analysis decisions (user) — D1–D7

1. **D1:** Accept F1's disposition (iteration-level offset = between-session
   variance, not a pipeline defect → no block rerun)? Includes the
   clarification pinning the A/A significant-finding (and halt) rule's scope:
   analysis-unit tests only, or any pipeline output.
2. **D2:** Taint semantics + session 6. The literal convention says tainted
   **session**; the pipeline excludes tainted **iterations**. Pin the rule
...
3. **D3:** Redefine the UDP pre-slope taint rule (F2). Candidates:
   ...
   should also be enumerated in the protocol if they are to carry
   rule weight in D2-style decisions.
4. **D4:** Pin H1's outcome operationalization — "median
   ...
   **The SESOI-exceeds-band requirement currently holds only under the
   ...
   before the n is set. The two scripts consolidate to whichever wins.
5. **D5:** Per-cell n: **8** (recommended) vs 11 (covers H2(b) @ 60 %
   shrinkage).
6. **D6:** H2(b)'s planning test: literal form (median ≥50 %
...
7. **D7:** hotelReservation gate sufficiency. What exists: solver gate
   ...
   downgraded procedure as a deviation), or run the live gate
   before settling, (~1 h, cluster is up).
...
- **Layered sub-scores (H5)** are not implemented in the package; their
  *definitions* are fixed at M2 regardless (circularity mitigation).
...
- **Analysis consolidation** per D4: one canonical extraction shared by the
  ...
  with the primary tests (absolute UDP drop, not the pct ratio).

## M2 exit criteria status (workplan)

| Criterion | Status |
|---|---|
| A/A block clean (no unexplained significant findings) | ✅ analysis-unit tests all n.s.; F1 explained, accepted (D1, no rerun) |
...
| All TBDs resolved | ✅ D1–D7 decided (all per recommendation, 2026-06-12); every TBD filled in |
| Power analyses against each SESOI | ✅ this report (H1 under the D4-selected supplementary extraction) |
| Wording clarifications (M1b carry-overs + D1–D4 scoping + block-design instantiation) | ✅ recorded in this report |
| Analysis decisions settled by commit hash | ⏳ blocked on D1–D7 |
