...
>
> That is why **L3 is inapplicable here on its own terms, rather than refuted by a correlation.** Recovery-time decomposition did not provide a stable ranking signal here: the split between its two phases is itself unstable run-to-run, and the resilience score it would supposedly predict is non-reproducible. There is no stable relationship to find on either side in this setup.
>
