...
>
> That is why **L3 is refuted structurally, not by a correlation.** Recovery time can't predict the resilience outcome: the split between its two phases is itself unstable run-to-run, and the resilience score it would supposedly predict is non-reproducible. There is no stable relationship to find on either side.
>
...
>
> **L3 -- Recovery time predicts resilience: refuted structurally.** The split between recovery's two phases is itself unstable run-to-run -- application startup dominates in some runs, the scheduling term in others -- and the score it would predict is non-reproducible. There is no stable relationship on either side.
>
