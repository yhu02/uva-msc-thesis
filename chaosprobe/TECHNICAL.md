...
| `solve(edges, services, n_nodes, target_f, capacity=None, seed=0, node_capacity=None)` | Greedy edge-cut assignment toward target `f` + local search (single moves + pairwise swaps), optional requests-based capacity; returns a `Solution` with the acceptance verdict (\|achieved − target\| ≤ 0.05) |
