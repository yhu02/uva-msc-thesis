% Chapter 3 — the ChaosProbe framework.
...
commands are pure functions of the stored output: every statistic in Chapter~\ref{ch:results} can be recomputed from an archived \texttt{summary.json} without cluster access.
...
study does not select among them directly but drives placement
...
the realized placement verifiable: the $f$ knob varies between-service topology
at one replica per service, and the $r$ knob reaches $r{=}3$ packed/anti-affine
through round-robin per-replica pinning in H3's node-drain campaign
(\S\ref{sec:h3}). Multi-replica anti-affinity beyond that node-drain regime is
out of reach of the current mutator, a boundary that shapes the future-work
