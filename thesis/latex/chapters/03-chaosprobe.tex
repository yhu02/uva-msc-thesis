...
\texttt{dependency-aware} (a BFS partition of the service dependency graph; \S\ref{sec:scheduler-lineage}
gives each strategy's literature anchor). Because a \texttt{nodeSelector} is a
...
rather than silently polluting it (\S\ref{sec:threats-table}). This deployment-level,
single-replica design is a scoping decision with consequences the thesis
returns to repeatedly: it cleanly manipulates \emph{between-service} topology, but
it structurally excludes multi-replica anti-affinity questions (\S\ref{sec:generalizes}, \S\ref{sec:future-work}).
...
whose run-to-run instability is itself a finding (\S\ref{sec:l1l3}). When a Litmus
probe returns an \texttt{Unknown} verdict, a diagnostics module captures the
cluster state at that moment, which is how the study can assert \emph{why} the
score is unusable under node drain (\S\ref{sec:h6}) rather than merely observing
that it is.
...
(under a node drain, every Litmus probe returns \texttt{Unknown} and the score is
unusable --- \S\ref{sec:h6}).
...
experiments require --- the regime that, as Appendix~\ref{app:negative} documents, synthetic
...
graph is storage; H5 makes it \textbf{analytically load-bearing}: the cross-node
call fraction --- H5's static predictor --- is computed by joining the graph's
inter-service edges with the recorded \texttt{podPlacements}, so the prediction
exists \emph{before any chaos is injected} and depends on the graph being a
faithful model of the application's call structure. The \texttt{summary.json}
...
\texttt{routeViewAggregate} from which the east-west inter-service tails of H4/H5
...
\texttt{scripts/score\_variance.py} (H1's variance partition, ICC, bootstrap CI,
power), \texttt{mechanism\_metrics.py} (H2), \texttt{h3\_mechanism\_outcome.py} (H3),
\texttt{cross\_node\_fraction.py} (H5), \texttt{blast\_radius.py} (H6), and
\texttt{campaign\_status.py} (session bookkeeping) --- each committed in the
...
--- co-location apparently $\sim$3$\times$ better at the user layer under load --- but the
run had untracked working-tree changes and incomplete metadata. When it was
replaced by two \texttt{doctor}-gated $i = 4$ batches, the user-layer effect
collapsed and only the mechanism-layer effect survived (\S\ref{sec:h4}). Had the dirty
