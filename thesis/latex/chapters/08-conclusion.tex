...
obscure those effects?} Answered through a Holm-corrected primary family with
effect-size bars set against a measured noise floor, the result is a real
mechanism whose reach is rigorously bounded.
...
The bound is what the strict bars make precise. No primary hypothesis
clears its bar (\S\ref{sec:family-holm}): the dose-response is
...
mechanism; at the effect-size bar, it does not move the user-visible
...
worse'' or a best strategy. ChaosProbe and the provenance-gated campaign
...
confirmatory re-run left as future work --- recovers the
...
and correcting it under the same provenance-gated discipline is
...
evaluation of placement needs \textbf{layered measurement, effect-size bars set
against a measured noise floor, and provenance-gated replication, not a single
score}: each fault class left its placement signal in a specific layer, a
one-number instrument's availability component was unreliable or undefined
exactly where it mattered, and holding the effects to a strict, noise-calibrated
bar is what turned ``we observed placement effects'' into the more honest ``here
is exactly which effects survive a strict bar, and which do not.''

\section{Future work}\label{sec:future-work}

\subsection*{Beyond the second workload}

The single largest threat (\S\ref{sec:generalizes}) --- that all primary
...
\item \textbf{A rescue-margin the depth face can express.} H3's trough-depth
  co-primary could not adjudicate rescue because the absolute one-pod
  ...
  depth\,$\times$\,duration metric) would let a future study test the
...
provenance-gated protocol this thesis contributes, and each
...
extension}: a second-environment direction-transfer arm (which
